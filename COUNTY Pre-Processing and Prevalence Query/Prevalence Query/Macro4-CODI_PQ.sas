...
%Macro Generate_Prev_Report(Infile, Pass, FailCode);
	...
	%If &Pass. = Y %Then %Do;
		Proc Sql;
			Create table Prevalence_Report as
				...
				Select 3 as Order, "Query Version: CODI, 2016-2019" as WtCat, Samp, Pop, Crde_Prev, Crde_StdErr, Wgt_Prev, Wgt_StdErr &Incld_Age_Adj_P2. From Null_Prev_Report Union
				...
				Select 17 as Order, "Suggested Citation: Colorado Health Observation Regional Data Service (CHORDS), 2016-2018. Accessed through prevalence query on &DateTime2." as WtCat, Samp, Pop, Crde_Prev, Crde_StdErr, Wgt_Prev, Wgt_StdErr &Incld_Age_Adj_P2. From Null_Prev_Report Union
	...
	%Else %Do;
		%If 		  &Failcode.=1 %Then %Let Fail_Desc=One or more demographic or geographic category has no groups selected. One or more group must be selected in each category. Ensure that each demographic and geographic category has one or more groups selected (e.g., age group, select an age range for inclusion). ;
			%Else %If &Failcode.=2 %Then %Let Fail_Desc=Years are out of scope for CODI. Acceptable years include 2016, 2017, 2018, and 2019 for CODI.;
			%Else %If &Failcode.=4 %Then %Let Fail_Desc=State and county are incorrectly specified. Review the lists and ensure each value is: Surrounded by quotations, comma delimited, and/or the correct length (e.g., "01", "02", "03", etc.).;
			%Else %If &Failcode.=5 %Then %Let Fail_Desc=Current selection criteria return an insufficient number of children and teens and do not meet minimum threshold to estimate sample weights. Ensure that selections are correct (e.g., correct list of state codes or county codes) or include additional geographic or demographic categories (e.g., add additional communities or include additional or all races, age groups, sex, etc.). ;
		...
		Proc Sql;
			Create table Prevalence_Report as
				...
				Select 3 as Order, "Query Version: CODI, 2016-2019" as WtCat, Samp, Pop, Crde_Prev, Crde_StdErr, Wgt_Prev, Wgt_StdErr &Incld_Age_Adj_F2. From Null_Prev_Report Union
...
Quit;
	...
	/*@Action: Output results report to CSV file ***/
