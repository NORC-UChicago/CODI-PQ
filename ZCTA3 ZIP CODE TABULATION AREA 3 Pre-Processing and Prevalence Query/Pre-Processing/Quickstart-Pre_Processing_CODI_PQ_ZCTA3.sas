/*****************************************************************************************/
/***PROGRAM: Quickstart-Pre_Processing_CODI_PQ_ZCTA3.SAS						  	   ***/
/***VERSION: 1.0																	   ***/
/***AUTHOR: SCOTT CAMPBELL (NORC at the University of Chicago)						   ***/
/***DATE CREATED: 7/20/2020, DATE LAST MOD: 7/24/2020								   ***/
/***INPUT: American Community Survey - ZCTA3-level (CSV)		   					   ***/
/***INPUT: EHR data file (CSV)														   ***/
/***INPUT: EHR data file with condition information (CSV) 							   ***/
/***OUTPUT: Pre-Processed American Community Survey -ZCTA3-Level (SAS)				   ***/
/***OUTPUT: Pre-Processed EHR SAS file (SAS)							   			   ***/
/***OBJECTIVE: PREPROCESSES CHILDHOOD and TEEN OBESITY EHR DATA and ACS ZCTA3 DATA     ***/
...
/***/ %LET PRE_DEST		= CODI_PQ_ZCTA3;					 /*@Note: Suffix name for EHR Output folder (ACCEPTABLE VALUES: folder name (no puctuation) 		      ***/
/***/ %LET ACS_FILENAME	= ACS_State_ZCTA3;	  			 /*@Note: ACS file name (ACCEPTABLE VALUES: file name, do not include ".csv") ***/
/***/ %LET EHR_FILENAME = NORC_IQVIA_Data;			   		 /*@Note: EHR file name (ACCEPTABLE VALUES: file name, do not include ".csv") ***/
/***/ %LET EHR_COND_FILENAME = NORC.Prevalence.Estimation_Co-occuring_Patient; /*@Note: EHR with condition data file name (ACCEPTABLE VALUES: file name, do not include ".csv") ***/
/***/ %LET LOG_NAME_PRE	= Quickstart_Pre_Processing_CODI_PQ_ZCTA3; /*@Note: SAS log file name prefix ACCEPTABLE VALUES: SAS file name (no punctuation) 	   ***/
...
/***/ %LET END_YEAR	  = 2018; /*@Note: LONGITUDINAL End year (ACCEPTABLE VALUES: 4-digit numeric year)   ***/

/*SECTION 3: OPTIONAL Output File Name Suffix																							   ***/
/***/ %LET EHR_PRE_Out  = CODI_PQ_ZCTA3; /*@Note: EHR output file name (ACCEPTABLE VALUES: SAS file name (no punctuation)		   			***/
...
%LET ACS_PRE_Out  = ACS_State_ZCTA3;	  	  /*@Note: Census ACS output file name (ACCEPTABLE VALUES: SAS file name (no punctuation) ***/
...
%LET MODULE1 =Module1-Pre_Processing_CODI_PQ_ZCTA3; /*@Note: Module 1 (ACCEPTABLE VALUES: name of SAS program, do not include ".sas") ***/
%LET MODULE2 =Module2-Pre_Processing_CODI_PQ_ZCTA3; /*@Note: Module 2 (ACCEPTABLE VALUES: name of SAS program, do not include ".sas") ***/
%LET MODULE3 =Module3-Pre_Processing_CODI_PQ_ZCTA3; /*@Note: Module 3 (ACCEPTABLE VALUES: name of SAS program, do not include ".sas") ***/

/*@Action: Check if optional input value was entered missing, if missing set default ***/
%Macro Check_Optional;
	%If "&EHR_PRE_Out." = "" %Then %Let EHR_PRE_Out = CODI_PQ_ZCTA3;
%Mend;
%Check_Optional;
...
Data SASOut.&ACS_PRE_Out.(Keep=Geography State_Alpha State_FIPS ZCTA3 &ACSNew.);
...
%Macro Import_EHR_Data;
	...
	Filename EHRCond "&ROOT_PRE.\00 Raw Data\&EHR_COND_FILENAME..csv";
	...
	Data Include_EHR(label="&EHR_PRE_Out.");
		...
		Attrib PATID length=$20 format=$20.  label="PATID: Patient ID"
			   %Do I=&Begin_Year. %to &End_Year. %by 1;
		   		   AGE&I.		length=4   format=4.	label="AGE&I.: Patient age (days) in &I."
				   SEX&I.		length=3   format=1.	label="SEX&I.: Patient sex in &I."
				   HEIGHT&I. 	length=6   format=8.4   label="HEIGHT&I.: Patient Height (cm) in &I."
				   WEIGHT&I. 	length=6   format=8.4   label="WEIGHT&I.: Patient Weight (kg) in &I."
				   AGEYR&I.		length=4   format=4.	label="AGE&I.: Patient age (years) in &I."
				   BMI&I.		length=6   format=8.4   label="BMI&I.: Patient BMI in &I."
				   ZSCORE&I.	length=6   format=8.4	label="ZSCORE&I.: BMI Z-Score in &I."
				   P95&I.		length=6   format=7.4	label="P95&I.: BMI 95th Percentile in &I."
				   WTCAT&I.  	length=3   format=1. 	label="WTCAT&I.: Patient weight category &I."
				   STATE&I.		length=$2  format=$2.	label="STATE: Patient state in &i."
				   ZIP&I.		length=3   format=3.  	label="ZIP: 3-Digit Zip Code"
			   %End;
			   MALE		length=4 format=4. label="MALE: Count of episodes indicating male"
			   FEMALE	length=4 format=4. label="FEMALE: Count of episodes indicating female"
			   BLACK	length=4 format=4. label="BLACK: Count of episodes indicating female"
			   ASIAN	length=4 format=4. label="ASIAN: Count of episodes indicating female"
			   WHITE	length=4 format=4. label="WHITE: Count of episodes indicating female"
			   HISPANIC length=4 format=4. label="HISPANIC: Count of episodes indicating female"
			   OTHER	length=4 format=4. label="OTHER: Count of episodes indicating female"
			   UNKNOWN  length=4 format=4. label="UNKNOWN: Count of episodes indicating female"
			   ;
		...
		Input PATID
			  ...
			  ;
			    ...
			    %Do I=&Begin_Year. %to &End_Year. %by 1;
					...
					If WTCAT&I.="" Then FLG_WTCAT&I.=0;
						Else FLG_WTCAT&I.=1;

				 	/*@Action: Assign weight category description ***/
					WTCAT&I._LIT = PUT(WTCAT&I., WgtCat.);
				%End;


				/*@Action: Reset summary variables to 0/1 flags ***/
				Array SumVar 8 MALE FEMALE BLACK ASIAN WHITE HISPANIC OTHER UNKNOWN;
				Array SumVarFlgs 8 MALE_FLG FEMALE_FLG BLACK_FLG ASIAN_FLG WHITE_FLG HISPANIC_FLG OTHER_FLG UNKNOWN_FLG;

				/*@Action: Create 0/1 race indication flags ***/
				Do i=1 to 8;
					If SumVar[i]>0 Then SumVarFlgs[i]=1;
						Else SumVarFlgs[i]=0;
				End;

				/*@Action: Sum number of indicated race values ***/
				Race_Tot = Black_flg+Asian_flg+White_flg+Hispanic_Flg+Other_flg+Unknown_flg;

				/*@Action: Set Indicated Race ***/
				Length Race $8.;
				If Race_Tot>1 Or Other_Flg=1 Then Race = "Other";
					Else IF Unknown_flg=1 or (Race_Tot=0 and Hispanic_Flg=0) Then Race = "Unknown";
					Else IF Black_flg=1 Then Race = "Black";
					Else IF Asian_flg=1 Then Race = "Asian";
					Else IF White_flg=1 Then Race = "White";
					Else IF Hispanic_Flg=1 Then Race = "Hispanic";

				/*@Action: Set Indicated Sex ***/
				Length Sex $8.;
				If Male_Flg=1 and Female_Flg=0 then Sex="Male";
					Else if Male_Flg=0 and Female_Flg=1 then Sex="Female";
					Else Sex="Unknown";
				Drop i;
	Run;
	...
	%Macro SQL_Var_Order;
		%Do I=%EVAL(&Begin_Year.+1) %to &End_Year. %by 1;
			AGEYR&I., Put(AGEYR&I., Age.) as AGE_CATEGORIES&I., HEIGHT&I., WEIGHT&I., BMI&I., WTCAT&I._LIT as WTCAT&I., FLG_WTCAT&I.,
			STATE&I. as STATE_ALPHA&I., Put(STATE&I., $ALPHA_to_FIPS.) as STATE_FIPS&I.,
			Case When ZIP&I. in (., -9) then "" Else Strip(Put(ZIP&I., Z3.)) end as ZCTA3&I.,
			Cats(Calculated STATE_FIPS&I., Calculated ZCTA3&I.) as Geography&I. Length=5 Format=$5.,
		%End;
	%Mend;

	/*@Action: Apply new ordering of variables and exclude any variables not needed for analysis ***/
	Proc Sql;
		Create table Include_EHR_Pre as
			Select PATID, %SQL_Var_Order RACE, SEX
				From Include_EHR;
	Quit;		
	...
	%Do I=%EVAL(&Begin_Year.+1) %To &End_Year. %By 1;
		Proc Sort Data=Include_EHR_Pre;
			By GEOGRAPHY&I.;
		Proc Sort Data=SASOut.&ACS_PRE_Out. Out=Sorted_ACS;
			Where TOTAL_ACS_POPULATION>0;
			By GEOGRAPHY;
		Data Include_EHR_Pre;
			Merge Include_EHR_Pre(In=A) Sorted_ACS(In=B Keep=GEOGRAPHY Rename=(GEOGRAPHY=GEOGRAPHY&I.));
				By GEOGRAPHY&I.;
					If A;
					If WTCAT&I.^="" and A and ^(B) then Bad_Geo_&I.=1;
						Else Bad_Geo_&I.=0;
		Run;
	%End;
	...
	Data Initial_EHR_PreProcess(Drop=Bad_Geo_: FLG_WTCAT:) Removed_EHR;
		Set Include_EHR_Pre;
			/*@Action: Exclude People outside 2-19 Ages Range ***/
			If (^(2<=Min(Of AGEYR%EVAL(&Begin_Year.+1)-AGEYR&End_Year.)<=19) and ^(2<=Max(Of AGEYR%EVAL(&Begin_Year.+1)-AGEYR&End_Year.)<=19)) or
				Sum(of FLG_WTCAT:)=0 or 
				Sum(of Bad_Geo_:)=Sum(of FLG_WTCAT:) Then Output Removed_EHR;
			Else Output Initial_EHR_PreProcess;
	Run;

	/*@Action: Load import file and convert into SAS dataset 		 ***/
	/*@Note: This file will be copied into output destination folder ***/
	Data EHR_Conditions(label="EHR Condition Information");
		Infile EHRCond dsd dlm="," lrecl=500 firstobs=2 stopover;

		Attrib PATID	  length=$20 format=$20.	  label="PATID: Patient ID"
			   SEX		  length=3   format=1.		  label="SEX: Patient sex"
			   AGE_GROUP  length=3   format=4.		  label="AGE GROUP: Patient age group"
			   ETHNICITY  length=$20 format=$20.	  label="ETHNICITY: Patient ethnicity"
	...
	/*@Action: Merge condition counts and dates with EHR Data file ***/
	Proc Sql;
		Create table EHR_Merged as
			select a.*, b.HCLCNT, b.CFCNT, b.SCDCNT, b.SBCNT, b.ASTHMACNT, b.CELIACCNT, b.SCZCNT
				From Initial_EHR_PreProcess as a Left Join EHR_Conditions as b
					On a.PATID=b.PATID;
	Quit;
	...
	Data &EHR_PRE_Out.;
		Set EHR_Merged;
			...
			Label HCL	 = "Hypercholesterolemia: Flag"
...
Data PreEHR_PQ_Pre;
	Set &EHR_PRE_Out.;
		...
		If WTCAT2018^="" Then do;
			...
			State_ZIP_Model=Cats(STATE_ALPHA2018, ZCTA32018);
			Geography=Geography2018;
			Sex_Model=Sex;
		End;
		Else If WTCAT2017^="" Then do;
			Age_Model=AgeYR2017;
			Height_Model=Height2017;
			State_ZIP_Model=Cats(STATE_ALPHA2017, ZCTA32017);
			Geography=Geography2017;
			Sex_Model=Sex;
		End;
		Else If WTCAT2016^="" Then do;
			Age_Model=AgeYR2016;
			Height_Model=Height2016;
			State_ZIP_Model=Cats(STATE_ALPHA2016, ZCTA32016);
			Geography=Geography2016;
			Sex_Model=Sex;
		End;
		Else Delete;
